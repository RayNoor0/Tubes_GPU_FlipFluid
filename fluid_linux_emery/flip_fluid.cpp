...
// Time a single pipeline stage into the optional StageTimers sink. When no sink
// is attached the call runs with zero overhead.
#define TIME_STAGE(STAGE, CALL)                                              \
    do {                                                                     \
        if (timers) {                                                        \
            auto _t0 = std::chrono::steady_clock::now();                     \
            CALL;                                                            \
            auto _t1 = std::chrono::steady_clock::now();                     \
            timers->add(flipbench::STAGE,                                    \
                         ...
                         int numSubSteps)
{
    ...
    for (int step = 0; step < numSubSteps; ++step) {
        TIME_STAGE(T1_integrate,  integrateParticles(sdt, gravity));
        if (separateParticles)
            TIME_STAGE(T2_pushApart, pushParticlesApart(numParticleIters));
        TIME_STAGE(T3_collisions, handleParticleCollisions(obstacleX, obstacleY,
                                       obstacleRadius, obstacleVelX, obstacleVelY));
        TIME_STAGE(T4_p2g,        transferVelocities(true));
        TIME_STAGE(T5_density, {
            updateParticleDensity();
            if (particleRestDensity == 0.0f)
                particleRestDensity = computeRestDensity();
        });
        TIME_STAGE(T6_pressure,   solveIncompressibility(numPressureIters, sdt,
                                       overRelaxation, compensateDrift));
        TIME_STAGE(T7_g2p,        transferVelocities(false, flipRatio));
    }
    TIME_STAGE(T8_colors, {
