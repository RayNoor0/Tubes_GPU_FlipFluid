...
// Resolutions required by the assignment's benchmark matrix.
static const int kBenchResolutions[] = {50, 100, 150, 200};

// Aggregate per-frame scalars used for CPU-vs-CUDA numeric validation.
struct SimStats {
    double meanSpeed;
    double totalKE;
    double centroidX;
...
static SimStats computeStats(const FlipFluid& f) {
    ...
    for (int i = 0; i < f.numParticles; ++i) {
        ...
        sumKE    += 0.5 * v2;               // unit particle mass
        cx += f.particlePosX[i];
        cy += f.particlePosY[i];
    }
...
// Render one frame without the UI overlay (used by the benchmark driver).
...
// Step the simulation with the fixed, deterministic benchmark config: static
// obstacle at its current (3,2) position with zero velocity.
static void stepFixed(FlipFluid& f) {
...
// Benchmark driver: for each resolution, warm up 60 frames then average the
// next 600 of steady-state per-stage timings, and append a CSV row.
static void runBenchmark(AppWindow& w, int singleRes) {
    const int warmup  = 60;
    const int measure = 600;

    std::FILE* csv = std::fopen("bench_cpu.csv", "w");
    ...
    for (int ri = 0; ri < nRes; ++ri) {
        ...
        setupScene();                 // gravity/separate/drift/flip defaults + obstacle(3,2)
        scene.paused = false;
        FlipFluid& f = *scene.fluid;
        ...
        std::printf("[flip-cpp] benchmarking res=%d particles=%d pIters=%d ...\n",
                    scene.resolution, f.numParticles, scene.numPressureIters);
        ...
        timers.numParticles = f.numParticles;
        timers.printSummary("cpu");
        if (csv) timers.writeCsvRow(csv, "cpu");
        f.timers = nullptr;
    }
    if (csv) std::fclose(csv);
    std::printf("[flip-cpp] benchmark complete -> bench_cpu.csv\n");
}

// Validation driver: dump per-frame aggregate scalars to CSV for CPU-vs-CUDA
// comparison.
...
int main(int argc, char** argv) {
    ...
    int  argRes    = 0;          // 0 => sweep all benchmark resolutions
    int  statFrames = 300;
    for (int i = 1; i < argc; ++i) {
        if (std::strcmp(argv[i], "--no-vsync") == 0) noVsync = true;
            ...
            std::printf("Usage: %s [--no-vsync] [--benchmark] [--dump-stats] [--res N] [--frames N]\n", argv[0]);
            std::printf("  --benchmark   run warmup(60)+measure(600) per-stage timing sweep -> bench_cpu.csv\n");
            std::printf("  --dump-stats  dump per-frame aggregate scalars -> stats_cpu.csv\n");
    ...
    // Headless-style measurement modes: run, dump CSV, exit (no UI loop).
    if (benchmark) {
