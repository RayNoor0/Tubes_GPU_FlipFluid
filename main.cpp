...
static void runBenchmark(AppWindow& w) {
    ...
    std::printf("[flip-cpp] benchmark: %d warmup + %d measured frames/resolution, vsync off\n",
                WARMUP, MEASURE);

    for (int ri = 0; ri < NRES && w.running; ++ri) {
        scene.resolution = resList[ri];
        setupScene();               // also re-seeds the static obstacle at (3,2)
        scene.paused = false;
        FlipFluid& f = *scene.fluid;
        ...
        for (int frame = 0; frame < WARMUP + MEASURE && w.running; ++frame) {
            while (XPending(w.dpy) > 0) {  // stay responsive; honor close/quit
            ...
            if (frame == WARMUP - 1)   // discard warmup
    ...
    std::printf("\n=== [CPU] FLIP per-stage benchmark (avg ms/frame) ===\n");
    ...
    std::printf("config: gravity ON, separateParticles ON, compensateDrift ON, "
                "flipRatio=%.2f, obstacle static @ (%.1f,%.1f)\n\n",
...
int main(int argc, char** argv) {
    ...
    while (w.running) {
        ...
        // T9e: UI overlay on top in pixel coords
        auto t9e_s = t9d_e;
        ...
        // ----- per-stage timing: T9 sub-stages, T9_render total, T_total -----
        auto frameEnd = t9f_e;
        ...
        fliptiming::stageAdd(ftim.timing, fliptiming::T9_render,     msSince(renderStart, frameEnd));
        fliptiming::stageAdd(ftim.timing, fliptiming::T9a_clear,     msSince(t9a_s, t9a_e));
        fliptiming::stageAdd(ftim.timing, fliptiming::T9b_grid,      msSince(t9b_s, t9b_e));
        fliptiming::stageAdd(ftim.timing, fliptiming::T9c_particles,  msSince(t9c_s, t9c_e));
        fliptiming::stageAdd(ftim.timing, fliptiming::T9d_obstacle,   msSince(t9d_s, t9d_e));
        fliptiming::stageAdd(ftim.timing, fliptiming::T9e_ui,         msSince(t9e_s, t9e_e));
        fliptiming::stageAdd(ftim.timing, fliptiming::T9f_swap,       msSince(t9f_s, t9f_e));
        fliptiming::stageAdd(ftim.timing, fliptiming::T_total,        msSince(frameStart, frameEnd));
